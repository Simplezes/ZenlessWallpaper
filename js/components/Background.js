...
export default class Background extends Component {
    ...
    render() {
        ...
        return `
            <div class="layout-container layout-${this.state.layout} ${this.state.isPortrait ? 'is-mobile' : 'is-desktop'}">
                ${isCalendar ? this.renderCalendarLayout() : this.renderDefaultLayout()}
                ${this.renderHalftoneOverlay()}
            </div>
        `;
    }
                ...
                <svg id="logo-svg" viewBox="0 0 2438 1001.7" preserveAspectRatio="xMidYMid meet" style="${this.getDesktopStyles()}">
                    ...
                    <g style="transform: translate(var(--box-bottom-tx), var(--box-bottom-ty));">
                        <foreignObject x="20" y="694" width="700" height="299">
                            ${this.renderUIBoxContent(monthData.num)}
                        </foreignObject>
                    </g>
            ...
            <div id="logo-container" class="mobile-view">
                <svg id="logo-svg" viewBox="0 0 1080 2400" preserveAspectRatio="xMidYMid meet" style="${this.getPortraitStyles()}">
                    ...
                    <g style="transform: translate(var(--m-label-tx), var(--m-label-ty));">
                        <foreignObject x="50" y="1500" width="1000" height="300">
                            ${this.renderCalendarHeader(monthData.num)}
                        </foreignObject>
                    </g>

                    <g style="transform: translate(var(--m-name-tx), var(--m-name-ty)) scale(var(--m-name-s));">
                        <foreignObject x="50" y="1700" width="1000" height="200">
                    ...
                    <!-- Global Patch Group (In front of everything) -->
                    <g style="transform: translate(var(--m-center-tx), var(--m-center-ty)) scale(var(--m-center-s));">
    ...
    renderUIBoxContent(monthNum) {
        return `
            <div class="calendar-modular-column">
                ${this.renderCalendarHeader(monthNum)}
                ${this.renderAgentName()}
            </div>
        `;
    }

    renderCalendarHeader(monthNum) {
        return `
            <div class="calendar-header-block">
                ...
                <div class="calendar-sub-label">
                    ...
                    <div class="calendar-sub-content">
                        <span class="calendar-zenless-label">ZENLESS — ${monthNum}</span>
    ...
    renderAgentName() {
        ...
        let fontSize = this.state.isPortrait ? '10vw' : '92px';
        if (name.length > 24) {
            fontSize = this.state.isPortrait ? '5.2vw' : '44px';
        } else if (name.length > 20) {
            fontSize = this.state.isPortrait ? '6.2vw' : '52px';
        } else if (name.length > 17) {
            fontSize = this.state.isPortrait ? '7.2vw' : '64px';
        } else if (name.length > 13) {
            fontSize = this.state.isPortrait ? '8.5vw' : '76px';
        } else if (name.length > 10) {
            fontSize = this.state.isPortrait ? '9.5vw' : '86px';
        }

        const maxWidth = this.state.isPortrait ? '62%' : '650px';
        ...
        return `
            <div class="calendar-footer-block">
                ...
                <div class="calendar-agent-name" style="font-size: ${fontSize}; max-width: ${maxWidth}; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">${name}</div>
            </div>
        `;
    }
    ...
    renderMonthDisplayContent(monthData) {
        return `
            <div class="calendar-month-center-display-svg">
