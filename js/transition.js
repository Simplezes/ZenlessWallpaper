(function () {
    ...
    const CHARACTER_DURATION = 4800;
    const BG_DURATION = 1500;
    const COLOR_DURATION = 100;
    const TRANSITION_OUTLINE_THICKNESS = 8;
    const TRANSITION_TYPES = ['shutter', 'cornerWaves', 'triangle', 'wavyCircle', 'parallelogram', 'highContrastCutIn'];
    ...
    class CharacterTransition {
        ...
        render() {
            ...
            bgCtx.save();
            bgCtx.fillStyle = `color-mix(in srgb, ${oldAccent}, black 24%)`;
            bgCtx.fillRect(0, 0, W, H);

            let pBG = Math.min(1, p * 1.2);

            if (this.currentType === 'wavyCircle' || this.currentType === 'parallelogram') {
                pBG = p;
            }

            this.definePath(bgCtx, W, H, pBG, 'inner');
            bgCtx.clip();
            bgCtx.fillStyle = `color-mix(in srgb, ${accent}, black 24%)`;
            bgCtx.fillRect(0, 0, W, H);
            bgCtx.restore();
        ...
        renderCutIn(W, H, p, accent) {
            ...
            const oldAccent  = this.colors.old || '#111111';
            const BG_OLD     = `color-mix(in srgb, ${oldAccent}, black 24%)`;
            const BG_NEW     = `color-mix(in srgb, ${accent}, black 24%)`;
            const P3_CYAN    = '#00D4FF';
            ...
            for (let i = 0; i < N_PNL; i++) {
                ...
                bgCtx.fillStyle = (i % 2 === 0)
                    ? `color-mix(in srgb, ${oldAccent}, black 38%)`
                    : BG_OLD;
                bgCtx.fill();
        ...
        definePath(ctx, W, H, p, variant = 'main') {
            ...
            switch (this.currentType) {
                ...
                case 'cornerWaves': {
                    ...
                    let rP = p;
                    if (variant === 'inner') rP = Math.pow(p, 1.4);
                    const r = maxR * rP;
                    if (r <= 0) return;
                ...
                case 'parallelogram': {
                    const skew = 300;
                    const sweepRange = W + skew + 400;

                    const p1 = p;
                    const w1 = sweepRange * p1;
                    const startX1 = -skew;
                    ctx.moveTo(startX1, 0);
                    ctx.lineTo(startX1 + w1, 0);
                    ctx.lineTo(startX1 + w1 - skew, H);
                    ctx.lineTo(startX1 - skew, H);
                    ctx.closePath();

                    if (variant === 'main') {
                        const p2 = Math.pow(p, 1.2);
                        const w2 = sweepRange * p2;
                        const startX2 = -skew - 50;
                        ctx.moveTo(startX2, 0);
                        ctx.lineTo(startX2 + w2, 0);
                        ctx.lineTo(startX2 + w2 - skew, H);
                        ctx.lineTo(startX2 - skew, H);
                        ctx.closePath();
                    }
                    break;
                }
        ...
        applyAccents(ctx, W, H, p, accent) {
            ...
            switch (this.currentType) {
                ...
                case 'parallelogram': {
                    ...
                    ctx.strokeStyle = accent;
                    ctx.lineWidth = 20 * (1 - p);
                    const edgeX2 = -skew - 50 + (sweepRange * Math.pow(p, 1.2));
                    ctx.beginPath();
                    ctx.moveTo(edgeX2, 0);
                    ctx.lineTo(edgeX2 - skew, H);
                    ctx.stroke();
                    break;
                }
        ...
        run(oldImg, newImg, options = {}) {
            ...
            this.colors.old = options.oldAccent || '#111111';
